...
<div class="px-6 py-4 space-y-6">
    ...
    <style>
        .details-header { background:#fff; border-radius:12px; padding:24px; box-shadow:0 1px 3px rgba(0,0,0,.05); border:1px solid #f1f5f9; margin-bottom:24px; }
        .header-meta { display:flex; gap:40px; flex-wrap:wrap; }
        .meta-group label { display:block; font-size:11px; font-weight:600; color:#94a3b8; text-transform:uppercase; letter-spacing:.05em; margin-bottom:4px; }
        .meta-group .value { font-size:15px; font-weight:600; color:#1e293b; }
        .btn-pay { background:#10B981; color:#fff; font-weight:600; padding:8px 16px; border-radius:8px; display:inline-flex; align-items:center; gap:8px; font-size:13px; transition:all .2s; border:none; cursor:pointer; }
        .btn-pay:hover { background:#059669; }
        .btn-pay:disabled { background:#D1D5DB; cursor:not-allowed; }
        .report-table th, .report-table td { border:1px solid #E5E7EB; padding:8px 12px; text-align:right; vertical-align:middle; }
    ...
    <div class="details-header flex justify-between items-end">
        <div class="header-meta">
            <div class="meta-group"><label>Periode</label><div class="value">{{ $tagihan->periode }}</div></div>
            <div class="meta-group"><label>Tanggal Generate</label><div class="value">{{ \Carbon\Carbon::parse($tagihan->tgl_generate)->format('d M Y') }}</div></div>
            <div class="meta-group"><label>Total Tagihan (Potongan)</label><div class="value text-green-700">Rp {{ number_format($tagihan->total_amount, 0, ',', '.') }}</div></div>
            <div class="meta-group"><label>Keterangan</label><div class="value">{{ $tagihan->keterangan }}</div></div>
            ...
            <table class="w-full report-table" style="border-collapse:collapse;">
                <thead style="opacity:.8;">
                    <tr>
                        ...
                        <th class="col-pinjaman">PINJAMAN GAJI</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="cell-name" style="color:#9CA3AF;text-decoration:line-through;">{{ $del->anggota->nama_anggota ?? 'Unknown' }}</td>
    ...
    <form method="POST" action="{{ route('penagihan.bayar') }}">
        ...
        <div class="bg-white rounded-xl border border-blue-200 overflow-hidden shadow-sm">
            <div class="px-6 py-4 flex justify-between items-center bg-blue-50 border-b border-blue-100">
                <div>
                    <h3 class="font-bold text-sm text-blue-800 tracking-wide uppercase">Tagihan Mandiri — {{ $tagihan->periode }}</h3>
                    <p class="text-xs text-blue-500 mt-0.5">Cicilan pinjaman dibayar langsung oleh anggota ke koperasi</p>
                </div>
                <div style="display:flex;gap:8px;">
                    <button type="submit" class="btn-pay" style="background:#1D4ED8;"
                        onclick="return confirm('Proses pembayaran cicilan mandiri untuk anggota yang dipilih?')"
                        {{ $activeDetails->where('status', '!=', 'Lunas')->count() == 0 ? 'disabled' : '' }}>
                    ...
                    <a href="{{ route('penagihan.exportExcel', $tagihan->id) }}" style="border:1px solid #BFDBFE;background:#EFF6FF;color:#1D4ED8;padding:7px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;">Export Excel</a>
                </div>
            </div>
                    ...
                    <thead>
                        <tr>
                            <th style="width:40px;text-align:center;">
                                <input type="checkbox" id="checkAllMandiri" onclick="document.querySelectorAll('.row-check-m:not([disabled])').forEach(c=>c.checked=this.checked)" class="w-4 h-4 cursor-pointer">
                            </th>
                            <th style="text-align:left;min-width:220px;">NAMA ANGGOTA</th>
                            <th class="col-pinjaman">CICILAN MANDIRI<br>{{ strtoupper($tagihan->periode) }}</th>
                            <th>STATUS</th>
                            <th>AKSI</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        @php $totalMandiri = 0; @endphp
                        @forelse($activeDetails as $detail)
                            @php $totalMandiri += $detail->jumlah_pinjaman; @endphp
                            <tr>
                                <td style="text-align:center;">
                                    <input type="checkbox" name="detail_ids[]" value="{{ $detail->id }}" class="row-check-m w-4 h-4 cursor-pointer" {{ $detail->status == 'Lunas' ? 'disabled' : '' }}>
                                </td>
                                <td class="cell-name">
                                    {{ $detail->anggota->nama_anggota ?? 'Unknown' }}
                                    <div style="font-size:10px;color:#1D4ED8;font-weight:normal;">NIK: {{ $detail->anggota->nik ?? '-' }}</div>
                                </td>
                                <td style="color:#B91C1C;font-weight:700;font-size:13px;">
                                    Rp {{ number_format($detail->jumlah_pinjaman, 0, ',', '.') }}
                                </td>
                                <td style="text-align:center;">
                                    @if($detail->status == 'Lunas')
                                        <span style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:12px;font-size:10px;background:#DEF7EC;color:#03543F;font-weight:700;">
                                    ...
                                    @else
                                        <span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:10px;background:#DBEAFE;color:#1D4ED8;font-weight:700;">BELUM BAYAR</span>
                                    @endif
                                </td>
                                <td style="text-align:center;">
                                    @if($detail->status != 'Lunas')
                                    <button type="button"
                                        onclick="if(confirm('Hapus tagihan mandiri atas nama {{ $detail->anggota->nama_anggota }}?')) document.getElementById('delete-{{$detail->id}}').submit()"
                                        style="color:#DC2626;padding:4px;border-radius:4px;" title="Hapus Tagihan">
                                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                    @else
                        ...
                        @empty
                            <tr><td colspan="5" style="text-align:center;padding:40px 16px;color:#9CA3AF;">Tidak ada anggota dengan pinjaman mandiri aktif.</td></tr>
                        @endforelse
                    </tbody>
                    @if($activeDetails->count() > 0)
                    <tfoot>
                        <tr style="background:#EFF6FF;font-weight:700;color:#1D4ED8;">
                            <td colspan="2" style="border:1px solid #E5E7EB;padding:10px 12px;font-size:13px;text-align:center;">TOTAL CICILAN MANDIRI</td>
                            <td style="border:1px solid #E5E7EB;padding:10px 12px;font-size:14px;color:#B91C1C;">Rp {{ number_format($totalMandiri, 0, ',', '.') }}</td>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            <div class="px-6 py-4 flex justify-between items-center bg-white border-b border-gray-200">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Hanya pinjaman dengan metode <strong>Potongan Gaji</strong> yang masuk tagihan ini</p>
                </div>
                <div style="display:flex;gap:8px;">
                    <button type="submit" class="btn-pay"
                        onclick="return confirm('Proses pembayaran untuk anggota yang dipilih?')"
                        {{ $activeDetails->where('status', '!=', 'Lunas')->count() == 0 ? 'disabled' : '' }}>
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Proses Potongan Terpilih
                    </button>
                    <a href="{{ route('penagihan.exportExcel', $tagihan->id) }}" style="border:1px solid #D1D5DB;background:#fff;color:#374151;padding:7px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;">Export Excel</a>
                </div>
            </div>
                ...
                <table class="w-full report-table" style="border-collapse:collapse;">
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2" class="col-pinjaman">PINJAMAN GAJI<br>{{ strtoupper($tagihan->periode) }}</th>
                    ...
                    <tbody class="bg-white">
                        @php
                            ...
                            $totalSimpanan = $totalPinjaman = $totalGrand = 0;
                        @endphp
                        ...
                        @forelse($activeDetails as $detail)
                            ...
                            <tr>
                                ...
                                <td class="cell-name">
                                    {{ $detail->anggota->nama_anggota ?? 'Unknown' }}
                                    <div style="font-size:10px;color:#4e0101;font-weight:normal;">NIK: {{ $detail->anggota->nik ?? '-' }}</div>
                                </td>
                                ...
                                <td style="color:#B91C1C;">
                                    {{ $detail->jumlah_pinjaman > 0 ? number_format($detail->jumlah_pinjaman, 0, ',', '.') : '-' }}
                                </td>
                                ...
                                <td style="text-align:center;">
                                    ...
                                    @else
                                        <span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:10px;background:#FEE2E2;color:#991B1B;font-weight:700;">BELUM</span>
                                    @endif
                                </td>
                                <td style="text-align:center;">
                                    @if($detail->status != 'Lunas')
                                    <button type="button"
                                        onclick="if(confirm('Hapus tagihan atas nama {{ $detail->anggota->nama_anggota }}?')) document.getElementById('delete-{{$detail->id}}').submit()"
                                        style="color:#DC2626;padding:4px;border-radius:4px;" title="Hapus Tagihan">
                                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="10" style="text-align:center;padding:40px 16px;color:#9CA3AF;">
    ...
    @endif {{-- end @else Gabungan --}}

    {{-- Hidden delete forms (semua type) --}}
    @isset($activeDetails)
