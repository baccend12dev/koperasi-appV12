...
@section('title', 'Pencairan Penarikan Simpanan')
@section('page-title', 'Pencairan Penarikan Simpanan')
...
<div class="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
    <div class="px-6 py-4 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
        <h3 class="font-bold text-sm text-gray-800 tracking-wide uppercase">
            DAFTAR PENCAIRAN PENARIKAN SIMPANAN
            @if($tahun)
            ...
            <tbody id="wd-tbody">
                                    ...
                                    <div @click.away="openBayar = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                                        ...
                                        <div class="flex justify-between items-start mb-5">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500 mt-0.5">{{ $row->anggota?->nama_anggota }} · Penarikan Simpanan</p>
                                            </div>
                                        ...
                                        <div class="bg-emerald-50 rounded-xl p-4 mb-5 text-center">
                                            <div class="text-xs text-emerald-600 font-semibold mb-1 uppercase tracking-wider">NOMINAL PENARIKAN</div>
                ...
                <tr>
                    <td colspan="6" class="px-6 py-14 text-center text-gray-400">
                        ...
                        Tidak ada data penarikan simpanan untuk periode ini.
                    </td>
                </tr>
                @endforelse
    ...
    @if($listPengambilan->count() > 0)
    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-xs text-gray-400">
        <div>Total {{ $listPengambilan->count() }} Transaksi</div>
        <div>Total Nominal: <strong class="text-amber-700 font-bold">Rp {{ number_format($totalNominal, 0, ',', '.') }}</strong></div>
    </div>
    @endif
</div>
